...
struct Proposal{
    ...
    owner: candid::Principal,//wallet address
}
...
impl Storable  for Proposal {
    ...
    ic_stable_structures::storable::Bound::Bounded { max_size: (MAX_VALUE_SIZE), is_fixed_size: (false) };
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        ...
        match old_proposal_opt {
            ...
            None => return Err(VoteError::NoSuchProporsal)
        }
        if old_proposal.owner != ic_cdk::caller() {
            return Err(VoteError::AccessRejected);
        }
        
        let res = PROPOSAL_MAP.with(|p| p.borrow_mut().insert(key,old_proposal));
        match res {
            Some(_) => Ok(()),
